...
Name = '''BendCruvedHMSRod2'''

'''calculation settings'''
numTimeStep1 = 1
numTimeStep2 = 500
numTimeStep = numTimeStep1 + numTimeStep2 + 1
dt = []
T = []
...
for i in range(numTimeStep2):
    dt.append(0.5/numTimeStep2)
    T.append(T0 + 0.5/numTimeStep2 * (i+1))
T0 = T[-1]
dt.append(100000.0)
T.append(T0 + 100000.0)    
'''material setting'''
...
fList = []
for i in range(numTimeStep1):
    AMList.append([np.array([[0.00],[0.01],[0.0]]),np.zeros([3,3])])
    gList.append(np.array([[0],[0],[9.8]]))
    fList.append(np.zeros([6,1]))
for i in range(numTimeStep2+1):
    AMList.append([np.array([[0.000],[0.0],[0.0]]),np.zeros([3,3])])
    gList.append(np.array([[0],[0],[9.8]]))
    fList.append(np.zeros([6,1]))
...
for i in range(numTimeStep1):
    ...
    FLoading2.append([0.0,0.0,0.0])
    FLoading3.append([0.0,0.0,0.0])
    DLoading1.append(0.0)

   
for i in range(numTimeStep2+1):
    
    FLoading1.append(np.array([0.0007*Ki,0.0,0.0]))
    FLoading2.append(np.array([0.0010*Ki,0.0,0.0]))
    FLoading3.append(np.array([0.0007*Ki,0.0,0.0]))
    FLoading4.append(np.array([0.000000001,0.0,0.0]))
    '''
    FLoading1.append(np.array([0.0007,0.0,0.0]))
    FLoading2.append(np.array([0.0004,0.0,0.0]))
    FLoading3.append(np.array([0.0007,0.0,0.0]))
    '''
    DLoading1.append(0.0)
...
for i in range(len(nodes)):
    ...
    Fconstraints.append(Constraint([i,2],
                                   ...
                                   FLoading3,)) 
    '''
    Fconstraints.append(Constraint([i,4],
                                   'viscous boundary',
                                   FLoading4,))
    '''

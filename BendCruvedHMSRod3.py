...
for i in range(numTimeStep2):
    dt.append(0.4/numTimeStep2)
    T.append(T0 + 0.4/numTimeStep2 * (i+1))
T0 = T[-1]
dt.append(1000000.0)
T.append(T0 + 1000000.0)   
'''material setting'''

materials = [] 
materials.append(IsotropicLE(50000000,0.499,1741.23)) 
...
fList = []
for i in range(numTimeStep1):
    AMList.append([np.array([[0.01],[0.0],[0.0]]),np.zeros([3,3])])
    gList.append(np.array([[0],[0],[9.8]]))
    fList.append(np.zeros([6,1]))
for i in range(numTimeStep2+1):
    AMList.append([np.array([[0.00],[0.0],[0.0]]),np.zeros([3,3])])
    gList.append(np.array([[0],[0],[9.8]]))
    fList.append(np.zeros([6,1]))
...
for i in range(numElement):
    elements.append(HM3DClassicalRodL3([nodes[2*i],nodes[2*i+1],nodes[2*i+2]],1))
...
for i in range(numTimeStep2+1):
    FLoading1.append(np.array([0.001,0.0,0.0]))
    DLoading1.append(0.0)

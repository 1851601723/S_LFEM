...
Loading = np.ones(numIncrement)                                          # Loading path  
TLoading = np.array(range(numIncrement)) / float(numIncrement) 
...
'''Geometric features'''  
L = 90.0                                                                        # the length of the Robotic arm (unit: mm)   
...
materials.append(material.mLEIsotropy(0.1,0.50))                                 # (unit of modulus : N*mm^-2)                                                      
...
for i in range(numNode):
    nodes.append(node.n3DRoboticArm(np.array([[0.0],[0.0],[i * L / (numNode-1)]]),
                                        ...
                                        np.array([[0],[0],[0]]),
                                        [[15.0],[np.array([9.0,0.0])]],
                                        np.array([[0],[0],[0.004508],[0],[0],[0]])
                                        )
                 )

for i in range(numElement):
    elements.append(element.kLE3DRoboticArmL3([nodes[nodeList[i][0]],nodes[nodeList[i][1]],nodes[nodeList[i][2]]],
                                                  materials[0],
                                                  geometrys[0],
                                                  Loading,
                                                  [TLoading],

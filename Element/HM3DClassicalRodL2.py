...
class HM3DClassicalRodL2():
    
    '''3D hard-magnetic classical linear elastic rod L2 element'''
    
    def __init__(self,nodes,reduce):
        
        self.eps = 1e-7
        self.numNodes = 2                                                      # number of the nodes in the element
    ...
    def get_K_P(self,dt,beta,gamma):
        ...
        for g in self.guassPoints:
            ...
            H2[3:6,0:3] = base.V2skew(g.Rc) * g.Arho / (beta * dt**2)
            H2[3:6,0:3] = np.matmul(base.V2skew(g.ddR),base.V2skew(g.Rc)) * g.Arho
    ...
    class guassPoint():
        ...
        def __init__(self,element,guassLocation,guassWeight):
            ...
            self.N = np.array([[(1 - self.guassLocation) / 2],                 # node 1 
                               [(1 + self.guassLocation) / 2]])                # node 2
            ...
            self.omega01 = float(np.matmul(omega0.T,self.E1)) * 0.0            # initial curvature in E1 dirction
            self.omega02 = float(np.matmul(omega0.T,self.E2)) * 0.0            # initial curvature in E2 dirction

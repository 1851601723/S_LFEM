...
class HM3DClassicalRodL3():
    
    '''3D hard-magnetic classical linear elastic rod L3 element'''
        ...
        for g in self.guassPoints:
            ...
            for i in range(self.numNodes):
                ...
                for j in range(self.numNodes):
                    ...
                    self.Kd[i*self.numDof:(i+1)*self.numDof,j*self.numDof:(j+1)*self.numDof] += ((H1 + H2 * 0.0) * g.guassWeight * g.detJ * float(g.N[i,0]) * float(g.N[j,0]))

# -*- coding: utf-8 -*-
"""
Created on Sat May 22 20:15:21 2021
...
material = []
material.append(material_HE_NeoHookean(1.0,0.000001))
...
for i in range(n):
    for j in range(m):
        for k in range(m):
            element.append(element_3DS_HENH_L8([node[int(ID[i,j,k])],
                                                node[int(ID[i+1,j,k])],
...
for i in range(n+1):
    for j in range(m+1):
        D_constraints.append(constraint(int(ID[i,j,0]) * 3 + 2,'displacement boundary',[0],Loading))
        D_constraints.append(constraint(int(ID[i,j,-1]) * 3 + 2,'displacement boundary',[0],Loading))
